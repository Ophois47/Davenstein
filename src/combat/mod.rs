...
fn process_fire_shots(
    ...
    const ENEMY_HALF_H: f32 = 0.50;   // guard is ~1.0 tall in world
    ...
    fn roll_pistol_damage(tile_dist: i32, rng: &mut u32) -> i32 {
        // Wolf-ish buckets (tuned overall by Guard HP = 16 in your enemies.rs)
    ...
    fn ray_hit_vertical_cylinder(
        ...
        let a = d.dot(d);
        if a < 1e-8 {
            return None; // essentially vertical in XZ; ignore for now
        }
    ...
    for shot in shots.read() {
        ...
        for (e, occ) in q_alive.iter() {
            ...
            let Some(t) = ray_hit_vertical_cylinder(shot.origin, dir, center, ENEMY_RADIUS, ENEMY_HALF_H) else {
                continue;
            };
